...
# MAGIC %md ### IMPORTANT NOTE ABOUT reduceByKey() and reduce()
# MAGIC 
# MAGIC `reduceByKey()` is a transformation, whereas `reduce()` includes an action!
# MAGIC Looking at the source code for reduce(), notice the `collect()` 
# MAGIC https://spark.apache.org/docs/latest/api/python/_modules/pyspark/rdd.html#RDD.reduce
...
# MAGIC %md __INSTRUCTOR NOTES__   
# MAGIC The following code block is going to fail. The objective is to help students learn debugging. We'll also pay homage to Bob Ross. While designing this exercise, your lecturer, Kyle Hamilton, made some mistakes. But just like Bob, Kyle doesn't believe in mistakes, only happy accidents!!
# MAGIC 
# MAGIC Some questions to ask:   
# MAGIC 
# MAGIC Q: What does this error message tell us?   
# MAGIC A: list index out of bounds   
# MAGIC 
# MAGIC Q: How can we go about debugging the problem?   
# MAGIC A: Run each function to inspect the output. Is it what we are expecting? Does this make sense of the outofbounds error?    
# MAGIC What's odd about the output from the reduceByKey?   
...
# MAGIC 
# MAGIC Q: What other aggregation functions can we use here to solve this problem?   
# MAGIC A: Some things to try:
# MAGIC 1. FoldByKey - we get a default value to start with   
# MAGIC 2. CombineByKey - we have more control over how data is combined and reduced. Why didn't this work? reduceByKey calls combineByKey. We're performing the same operation as before, so this doesn't help fix our problem
# MAGIC 3. AggregateByKey - allows us to specify a null value, forcing the evaluation of the reduce step. How is this different from foldByKey?
...
studentAvgs = gradesRDD.map(lambda x: (x[0], (x[2], 1, x[2])))\
...
# MAGIC %md ### <--- SOLUTION --->  
# MAGIC __SOLUTION__   
# MAGIC Student 10004 is assigned an initial 'F' grade. We have not met the foldByKey requirement because our zeroValue changes the result
...
# MAGIC %md ### <--- SOLUTION --->  
# MAGIC __SOLUTION__   
# MAGIC We know that `reduceByKey` calls `combineByKey` under the hood. We have not changed anything here, so the result is exactly the same as in our first attempt when we used `reduceByKey`

# COMMAND ----------

# MAGIC %md ### aggregateByKey requires a null and start value as well as two different functions. One to aggregate within partitions, and one to aggregate across partitions
