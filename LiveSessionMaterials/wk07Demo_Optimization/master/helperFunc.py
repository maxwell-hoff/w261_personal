...
#     print(model)
    ...
    return OLSGradient(X,y,model) + np.append(reg_term,[0.0])
...
def compareDescent(X, y, hist1, hist2, labels):
    ...
    # add contour plots
    w0_min = min(hist1.T[0].min(), hist2.T[0].min()) - 0.5
    w1_min = min(hist1.T[1].min(), hist2.T[1].min()) - 0.5
    w0_max = max(hist1.T[0].max(), hist2.T[0].max()) + 0.5
    w1_max = max(hist1.T[1].max(), hist2.T[1].max()) + 1.5
...
def compareLossCurves(X, y, reg_type, reg_param, grid):
    ...
    print("loss no-reg", min(loss_no_reg))
    print("loss w-reg", min(loss_w_reg))